...
bool testErrorNames(){
    ...
    return true;
    // fix explanation : 
    //  the Original Code had mtm::CellOcupied and mtm::IllegalTarget which correct for naming the entire path
    //  however that's problematic for printing purposes.Therefore, I removed mtm:: from both lines(73,76).
...
bool testAttackSoldier(){
    ...
    //     }
    // }
    /* The Test above is commented because it's not correct. if a target is in range then it should legal to attack him
        we returned OutOfAmmo() which is what should happen in this specific case because the player doesn't have ammo 
        and he is attacking someone in his range(ditance 7, coordinates = (3,8) )*/
